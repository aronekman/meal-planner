  ...
  const summary: PlanSummary | null = useMemo(() => {
    ...
    return plan.meals.reduce(
      (prev, curr) => ({
        ...
        fat: prev.calories + curr.recipe.ingredients.reduce((prev, curr) => prev + curr.fat, 0)
      }),
      { cost: 0, protein: 0, calories: 0, fat: 0 }
    );
  ...
  return (
    <div>
      ...
      <div className='flex flex-col gap-4 p-2 px-4 pt-6'>
        ...
        <div className='flex h-30 flex-col rounded-lg bg-gray-600 bg-opacity-50 p-1'>
          ...
          <div className='flex flex-col h-full py-2 px-10 justify-center text-base font-alegreya'>
            ...
            <div className='flex justify-between'>
              <span className='text-white'>Fat (total)</span>
              <span className='text-white'>{summary ? summary.fat.toFixed(2) : 0.00} g</span>
            </div>
          </div>
        </div>
        <PlanCard />
